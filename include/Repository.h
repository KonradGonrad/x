...
public:
    ...
    void add(Ptr item) {
        if (!item) {
            throw ValidationException("Cannot add null item to repository");
        }
    ...
    Ptr getById(KeyType id) const {
        ...
        if (!item) {
            throw NotFoundException("Item with given ID");
        }
        return item;
    }
    ...
    bool remove(KeyType id) {
        auto it = std::remove_if(items.begin(), items.end(),
            [this, &id](const Ptr& item) {
                return item && getKey(item) == id;
            });
        if (it != items.end()) {
