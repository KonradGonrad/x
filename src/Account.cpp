...
void Account::deposit(double amount) {
    if (status != AccountStatus::ACTIVE) {
        throw AccountBlockedException(iban);
    }
    if (amount <= 0) {
        throw InvalidOperationException("Kwota wpłaty musi być większa od zera");
    }
    balance += amount;
}
...
void Account::withdraw(double amount) {
    ...
    if (amount <= 0) {
        throw InvalidOperationException("Kwota wypłaty musi być większa od zera");
    }
    if (amount > balance) {
